...
public class SemTaintSolver implements Solver {

    private static final Logger logger = LogManager.getLogger(SemTaintSolver.class);
    ...
    private ConfManager config = ConfManager.v();

    // ---- taint quick filtering ----

    /**
     * Global switch for taint fast filtering.
     */
    private boolean taintFilterEnabled = false;

    /**
     * Cache of taint objects by CSObj index.
     */
    private final BitSet taintBits = new BitSet();

    /**
     * Cache of checked CSObj indexes.
     */
    private final BitSet checkedBits = new BitSet();

    /**
     * Resolved lazily after plugins start.
     */
    private TaintManager taintManager;

    @SuppressWarnings("unchecked")
    public SemTaintSolver(AnalysisOptions options, HeapModel heapModel,
        ...
        ignoredMethods = Sets.newSet(); //必须在这里初始化
    }
    ...
    public void setTaintFilterEnabled(boolean enabled) {
        this.taintFilterEnabled = enabled;
    }
    ...
    /**
     * Resolve TaintManager from plugin chain after plugin.onStart().
     */
    private void resolveTaintManager() {
        if (!taintFilterEnabled) {
            return;
        }
        try {
            Field pluginField = this.getClass().getDeclaredField("plugin");
            pluginField.setAccessible(true);
            Object rootPlugin = pluginField.get(this);
            if (!(rootPlugin instanceof CompositePlugin compositePlugin)) {
                return;
            }

            Field allPluginsField = CompositePlugin.class.getDeclaredField("allPlugins");
            allPluginsField.setAccessible(true);
            Object allPluginsObj = allPluginsField.get(compositePlugin);
            if (!(allPluginsObj instanceof List<?> allPlugins)) {
                return;
            }

            for (Object p : allPlugins) {
                if (p instanceof TaintContextProvider provider) {
                    TaintManager mgr = provider.getTaintManager();
                    if (mgr != null) {
                        this.taintManager = mgr;
                        return;
                    }
                }
            }
    ...
    private boolean isTaintCSObj(CSObj csObj) {
        int idx = csObj.getIndex();
        if (checkedBits.get(idx)) {
            return taintBits.get(idx);
        }
        checkedBits.set(idx);
        if (taintManager != null && taintManager.isTaint(csObj.getObject())) {
            taintBits.set(idx);
            return true;
        }
        return false;
    ...
    /**
     * Excludes taint objects from pts.
     */
    private PointsToSet filterTaintObjects(PointsToSet pts) {
        boolean hasTaint = pts.objects().anyMatch(this::isTaintCSObj);
        if (!hasTaint) {
            return pts;
        }
        PointsToSet filtered = ptsFactory.make();
        pts.forEach(csObj -> {
            if (!isTaintCSObj(csObj)) {
                filtered.addObject(csObj);
            }
        });
        return filtered;
    }
            ...
            while (!workList.isEmpty() && !isTimeout) {
                ...
                if (entry instanceof WorkList.PointerEntry pEntry) {
                    ...
                    if (!diff.isEmpty() && p instanceof CSVar v) {
                        ...
                        /**
                         * move functionally related array store/load to plugin <li>@IndexedArrayModel</li>
                         */
                        if (!config.getBoolean("pta.container-enable",false)) {
    ...
    private PointsToSet propagate(Pointer pointer, PointsToSet pointsToSet) {
        ...
        if (!filters.isEmpty()) {
            if (taintFilterEnabled) {
                pointsToSet = filterTaintObjects(pointsToSet);
            } else {
                // apply filters (of the pointer) on pointsToSet
                pointsToSet = pointsToSet.objects()
                        .filter(o -> o != null && filters.stream().allMatch(f -> f.test(o)))
        ...
        for (Invoke callSite : var.getInvokes()) {
            pts.forEach(recvObj -> {
                ...
                if (callee != null) {
                    ...
                    if (!isIgnored(callee) && calleeContext != null
                            && callee.getIR() != null
                            && callee.getIR().getThis() != null) {
    ...
    private void processCallEdge(Edge<CSCallSite, CSMethod> edge) {
        if (callGraph.addEdge(edge)) {
            ...
            if (edge.getKind() != CallKind.OTHER
                ...
                if (calleeIR == null) {
                    logger.warn("Skip call edge without IR: callSite={}, callee={}", callSite, callee);
                ...
                int paramCount = calleeIR.getParams().size();
                if (argCount != paramCount) {
                    logger.warn(
                            "Argument/parameter size mismatch: callSite={}, callee={}, args={}, params={}. " +
                                    "Passing by min(args, params) to avoid analysis crash.",
                            callSite, callee, argCount, paramCount);
                }
                int passCount = Math.min(argCount, paramCount);
                ...
                if (lhs != null && propTypes.isAllowed(lhs)) {
                    ...
                    for (Var ret : calleeIR.getReturnVars()) {
    ...
    private void processNewMethod(JMethod method) {
        if (reachableMethods.add(method)) {
            ...
            //TODO some interface methods will be added considering analyzing completeness, like Repo/Mapper
...
//            return;
    ...
    // For completeness of call graph analysis, add interface/abstract methods
    public void addCSMethodWithoutProcess(CSMethod csMethod) {
        callGraph.addReachableMethodWithoutIR(csMethod);
    }
