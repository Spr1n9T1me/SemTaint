 ...
 * @description: 支持容器指针的CSManager实现
 * @author: springtime
 **/
public class ContainerCSManager implements IContainerCSManager {

    private final ContainerCSManager.PointerManager ptrManager = new ContainerCSManager.PointerManager();

    private final ContainerCSManager.CSObjManager objManager = new ContainerCSManager.CSObjManager();
    ...
    @Override
    public ConPointer getConPointer(CSObj containerObj, AccessPattern accessPattern) {
        return ptrManager.getConPointer(containerObj, accessPattern);
    }

    @Override
    public Collection<ConPointer> getConPointers() {
        return ptrManager.getConPointers();
    }

    @Override
    public Collection<ConPointer> getConPointersOf(CSObj containerObj) {
        return ptrManager.getConPointersOf(containerObj);
    }
    ...
    private static class PointerManager {
        ...
        private final TwoKeyMap<CSObj, AccessPattern, ConPointer> conPointers = Maps.newTwoKeyMap();
        ...
        private ConPointer getConPointer(CSObj containerObj, AccessPattern accessPattern) {
            return conPointers.computeIfAbsent(containerObj, accessPattern,
                (csObj, ap) -> new ConPointer(csObj, ap, counter++));
        }
        ...
        private Collection<ConPointer> getConPointers() {
            return conPointers.values();
        }

        private Collection<ConPointer> getConPointersOf(CSObj containerObj) {
            var pointers = conPointers.get(containerObj);
            return pointers != null ? pointers.values() : Set.of();
        }
