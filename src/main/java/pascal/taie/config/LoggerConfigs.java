...
public final class LoggerConfigs {
    ...
    public static void setOutput(File outputDir) {
        ...
        String logFileName = LOG_FILE.endsWith(".log")
                ? LOG_FILE.substring(0, LOG_FILE.length() - 4) + "-" + timestamp + ".log"
                : LOG_FILE + "-" + timestamp;
        // new a file appender
        FileAppender fileAppender = FileAppender
                ...
                .withFileName(new File(outputDir, logFileName).getAbsolutePath())
                .withAppend(false)
                .build();
