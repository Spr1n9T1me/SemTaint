...
import com.semtaint.frame.lifecycle.BeanPointer;
import com.semtaint.frame.lifecycle.BeanPointerNode;
...
public abstract class NodeManager implements Indexer<Node> {
    ...
    private final TwoKeyMap<Obj, AccessPattern, ConPointerNode> conPtr2Node = Maps.newTwoKeyMap();

    private final Map<BeanPointer, BeanPointerNode> beanPtr2Node = Maps.newMap(256);
    ...
    protected ConPointerNode getOrCreateConPointerNode(Obj base,
                                                       AccessPattern accessPattern) {
        return conPtr2Node.computeIfAbsent(base, accessPattern, (o, ap) -> {
            ConPointerNode node = new ConPointerNode(o, ap, nodeCounter++);
            nodes.add(node);
            return node;
        });
    }
    ...
    @Nullable
    public BeanPointerNode getBeanPointerNode(BeanPointer beanPointer) {
        return beanPtr2Node.get(beanPointer);
    }

    protected BeanPointerNode getOrCreateBeanPointerNode(BeanPointer beanPointer) {
        return beanPtr2Node.computeIfAbsent(beanPointer, bp -> {
            BeanPointerNode node = new BeanPointerNode(bp, nodeCounter++);
            nodes.add(node);
            return node;
        });
    }
