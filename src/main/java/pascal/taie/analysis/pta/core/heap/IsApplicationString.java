...
public class IsApplicationString implements Predicate<String> {
    ...
    public IsApplicationString() {
        // Collect all classes first to avoid concurrent modification
        var appClasses = World.get().getClassHierarchy()
            .applicationClasses()
            .collect(Collectors.toList());
        
        // Collect all methods first to avoid concurrent modification
        var allMethods = appClasses.stream()
            .map(JClass::getDeclaredMethods)
            .flatMap(methods -> new java.util.ArrayList<>(methods).stream())
            .filter(Predicate.not(JMethod::isAbstract))
            .collect(Collectors.toList());
        
        // Now process the collected methods
        stringsInApp = allMethods.stream()
            .map(JMethod::getIR)
            .map(IR::getVars)
            .flatMap(vars -> new java.util.ArrayList<>(vars).stream())
