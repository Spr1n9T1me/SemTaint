...
public class CSCallGraph extends AbstractCallGraph<CSCallSite, CSMethod> {
    ...
    public boolean addReachableMethod(CSMethod csMethod) {
        return addReachableMethod(csMethod,
                csMethod.getMethod().getIR() != null);
    }

    /**
     * Adds a reachable method that cannot provide IR to this call graph.
     */
    public boolean addReachableMethodWithoutIR(CSMethod csMethod) {
        return allReachableMethods.add(csMethod);
    }
    ...
    public Set<CSCallSite> getCallSitesIn(CSMethod csMethod) {
        ...
        if (!method.isAbstract()) //TODO interface invoke avoiding
