...
import com.semtaint.taint.TaintEntryHandler;
...
public class TaintAnalysis extends CompositePlugin {
    ...
    private static TaintFlowGraph currentTFG;
...
//        logger.info(config);
        ...
        if (ConfManager.v().getBoolean("pta.framework-enable"))
            addPlugin(new TaintEntryHandler(context));  // *** add entry here ***
    ...
    private void reportTaintFlows() {
        ...
        if (ConfManager.v().getBoolean("pta.print-log",true))
            taintFlows.forEach(logger::info);
        IOUtil.writeString2File(formatTaintFlowsBySink(taintFlows),
            new File(World.get().getOptions().getOutputDir(), "taint-flows.txt").getAbsolutePath());
                
        solver.getResult().storeResult(getClass().getName(), taintFlows);
        TaintManager manager = context.manager();
        TaintFlowGraph taintFlowGraph = new TFGBuilder(solver.getResult(), taintFlows, manager).build();
        currentTFG = taintFlowGraph;
    ...
    private static String formatTaintFlowsBySink(Set<TaintFlow> taintFlows) {
        Map<SinkPoint, List<TaintFlow>> flowsBySink = taintFlows.stream()
                .collect(Collectors.groupingBy(
                        TaintFlow::sinkPoint,
                        TreeMap::new,
                        Collectors.collectingAndThen(Collectors.toList(),
                                list -> list.stream()
                                        .sorted(Comparator.comparing(TaintFlow::sourcePoint))
                                        .toList())));

        StringBuilder sb = new StringBuilder();
        sb.append("Total taint flows: ").append(taintFlows.size()).append('\n');
        sb.append("Total sinks with taint flows: ").append(flowsBySink.size()).append("\n\n");

        int sinkIndex = 1;
        for (Map.Entry<SinkPoint, List<TaintFlow>> entry : flowsBySink.entrySet()) {
            SinkPoint sinkPoint = entry.getKey();
            List<TaintFlow> flows = entry.getValue();
            JMethod sinkMethod = sinkPoint.sinkCall().getMethodRef().resolveNullable();
            String sinkMethodText = sinkMethod != null
                    ? sinkMethod.getSignature()
                    : sinkPoint.sinkCall().getMethodRef().toString();

            sb.append("=== Sink ").append(sinkIndex++).append(" ===\n");
            sb.append("SinkPoint: ").append(sinkPoint).append('\n');
            sb.append("SinkMethod: ").append(sinkMethodText).append('\n');
            sb.append("FlowCount: ").append(flows.size()).append('\n');

            int flowIndex = 1;
            for (TaintFlow flow : flows) {
                sb.append("  [").append(flowIndex++).append("] ")
                        .append(flow).append('\n');
            }
            sb.append('\n');
        }
    ...
    @Nullable
    public static TaintFlowGraph getCurrentTFG() {
        return currentTFG;
    }
}
