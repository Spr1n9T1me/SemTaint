...
import com.semtaint.SemTaintAnalysis;
...
public class ResultProcessor implements Plugin {
    ...
    private static void logStatistics(PointerAnalysisResult result) {
        ...
        MethodContextElementStatistics contextStats =
                collectMethodContextElementStatistics(result);
        logger.info(String.format("%-30s%s", "#context elements (method):",
                format(contextStats.getTotal())));
        logger.info(String.format("%-30s%s (obj) / %s (call) / %s (type) / %s (origin) / %s (other)",
                "#context elements by kind:",
                format(contextStats.getObj()),
                format(contextStats.getCallSite()),
                format(contextStats.getType()),
                format(contextStats.getOrigin()),
                format(contextStats.getOther())));
        logger.info("----------------------------------------");
    }

    private static MethodContextElementStatistics collectMethodContextElementStatistics(
            PointerAnalysisResult result) {
        Set<JMethod> originMethods = SemTaintAnalysis.getOriginMethods();
        Set<Object> totalElems = new HashSet<>();
        Set<Object> objElems = new HashSet<>();
        Set<Object> callElems = new HashSet<>();
        Set<Object> typeElems = new HashSet<>();
        Set<Object> originElems = new HashSet<>();
        Set<Object> otherElems = new HashSet<>();

        result.getCSCallGraph().reachableMethods().forEach(csMethod -> {
            Context context = csMethod.getContext();
            for (int i = 0; i < context.getLength(); ++i) {
                Object elem = context.getElementAt(i);
                totalElems.add(elem);
                if (elem instanceof Obj) {
                    ...
                    typeElems.add(elem);
                } else if (elem instanceof JMethod && originMethods.contains(elem)) {
                    originElems.add(elem);
                } else {
                    otherElems.add(elem);
                }
            }
        ...
        return new MethodContextElementStatistics(
                totalElems.size(),
                objElems.size(),
                callElems.size(),
                typeElems.size(),
    ...
    private static class MethodContextElementStatistics {

        private final long total;

        private final long obj;

        private final long callSite;
        ...
        private MethodContextElementStatistics(long total, long obj, long callSite,
                                               long type, long origin, long other) {
            this.total = total;
            this.obj = obj;
