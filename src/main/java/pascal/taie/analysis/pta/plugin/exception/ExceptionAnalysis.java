...
public class ExceptionAnalysis implements Plugin {
    ...
    private Set<CSObj> analyzeIntraUncaught(
        ...
        Map<Stmt, List<ExceptionEntry>> methodCatchers = catchers.get(method);
        if (methodCatchers == null) {
            if (method.getIR() == null) {
                return newExceptions;
            }
            methodCatchers = CatchAnalysis.getPotentialCatchers(method.getIR());
            // Handle case where getPotentialCatchers returns null
            if (methodCatchers == null) {
                return newExceptions;
            }
            catchers.put(method, methodCatchers);
        }
        List<ExceptionEntry> entries = methodCatchers.get(currentStmt);
        if (entries != null) {
